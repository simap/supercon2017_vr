 ...
 *  Mike Harrison 24/7/17 - added optimise_spi32 optimisation option to speed up SPI transfers using 32 bit mode
 * 10/11/17 - added kickwatchdog to avoid WDT timeout on big cards
********************************************************************/
...
BYTE MDD_SDSPI_AsyncReadTasks(ASYNC_IO* info)
{
    ...
    kickwatchdog; // some writes can take a couple of secs on big cards
...
#else
// reduce SPI deadtime by reading 32 bit words
unsigned char d;
BYTE* endPointer;

union {unsigned long l;unsigned char c[4];} bword;


 if(localCounter != 0x0000) {
      
 				    endPointer=localPointer+512;
                    SPICON1|=0x800; // 32 bit
                    
                 do {
                      SPIBUF=0xFFFFFFFF;
                      while(!SPISTAT_RBF); bword.l=SPIBUF;SPIBUF=0xFFFFFFFF;
                    *localPointer++ = bword.c[3];*localPointer++ = bword.c[2];*localPointer++ = bword.c[1];*localPointer++ = bword.c[0];
                      while(!SPISTAT_RBF); bword.l=SPIBUF;SPIBUF=0xFFFFFFFF;
                    *localPointer++ = bword.c[3];*localPointer++ = bword.c[2];*localPointer++ = bword.c[1];*localPointer++ = bword.c[0];
                      while(!SPISTAT_RBF); bword.l=SPIBUF;SPIBUF=0xFFFFFFFF;
                    *localPointer++ = bword.c[3];*localPointer++ = bword.c[2];*localPointer++ = bword.c[1];*localPointer++ = bword.c[0];
                      while(!SPISTAT_RBF); bword.l=SPIBUF;
                    *localPointer++ = bword.c[3];*localPointer++ = bword.c[2];*localPointer++ = bword.c[1];*localPointer++ = bword.c[0];
     
                    } while(localPointer!=endPointer);    
              
                   SPICON1&=~0x800; // back to 8 bit
...
void Delayms(BYTE milliseconds)
{
   ...
   delayus((unsigned int) milliseconds*1000);
...
{
    SPIENABLE=0;
    		//OpenSPI(SPI_START_CFG_1, SPI_START_CFG_2); // already set up 
    	    SPIBRG = spiclk_slow;
    SPIENABLE=1;
